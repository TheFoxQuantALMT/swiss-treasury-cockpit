...
from cockpit.config import CURRENCY_TO_OIS, PRODUCT_RATE_COLUMN, ECHEANCIER_INDEX_TO_WASP, SHOCKS, FLOAT_NAME_TO_WASP
...
from cockpit.data.parsers import _month_columns
...
def compute_daily_pnl(
    ...
    """Vectorized daily P&L: Nominal × (OIS − RateRef) / MM."""
...
def aggregate_to_monthly(
    ...
    pnl_arr = np.zeros((n_deals, n_months), dtype=np.float64)
    nom_avg_arr = np.zeros((n_deals, n_months), dtype=np.float64)
    ...
    for j, m in enumerate(unique_months):
        ...
        pnl_arr[:, j] = daily_pnl[:, mask].sum(axis=1)

        # Nominal-days: sum of daily nominals (for rate weighting)
        nom_days = nominal_daily[:, mask].sum(axis=1)
        nom_days_arr[:, j] = nom_days

        # Nominal: average over calendar days in the month
        nom_avg_arr[:, j] = nom_days / n_cal_days if n_cal_days > 0 else 0.0

        # OISfwd and RateRef: nominal-weighted average
...
def compute_strategy_pnl(monthly: pd.DataFrame) -> pd.DataFrame:
    ...
    1. Pre-aggregate by (Périmètre, Strategy, Currency, Product, Direction, Month, DIM)
    ...
    idx_cols = ["Périmètre TOTAL", "Strategy IAS", "Currency", "Month", "Days in Month"]
    ...
    base_cols = ["Périmètre TOTAL", "Strategy IAS", "Currency", "Month"]
...
def compute_book2_mtm(
    ...
    """
    try:
        from cockpit.engine.pnl.curves import wt  # reuse the WASP_TOOLS_PATH-aware import
...
def merge_results(
    ...
    - IAM/LD-HCD / IAM/LD-NHCD: exclude Direction B
    """
    parts = []
    ...
    if strategy is not None and not strategy.empty:
        ...
        exclude_iam = iam_mask & strategy["Direction"].isin(["B"])
...
def _resolve_rate_ref(deals: pd.DataFrame) -> pd.DataFrame:
    ...
    # MTM perimeter fallback: if product not in map and Périmètre = "MTM" → Clientrate
...
def _build_ois_matrix(
    ...
    """Map each deal to its currency's OIS daily curve → (n_deals × n_days)."""
    n_deals = len(deals)
    n_days = len(days)
...
def run_all_shocks(
    ...
    for shock in shocks:
        ...
        monthly = aggregate_to_monthly(daily_pnl, nominal_daily, ois_matrix, rate_matrix, days)
