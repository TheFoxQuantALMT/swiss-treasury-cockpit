...
- **`engine/`** — Computation core
  - `pnl/`: Vectorized P&L engine. `forecast.py` is the stateful entry point (`ForecastRatePnL`). `engine.py` has the functional core: daily P&L = `Nominal × (OIS - RateRef) × d_i / MM` (where `d_i` is the calendar-day weight per fixing built from the Swiss business calendar), aggregated to monthly, with IAS hedge strategy decomposition into 4 synthetic legs. `matrices.py` builds the numpy arrays (nominal schedule, alive mask, rate matrix). `curves.py` handles OIS curve loading with WIRP mock fallback when WASP is unavailable.
...
- **`pnl_dashboard/`** — Dedicated P&L dashboard (36 tabs). Uses Jinja2 + Chart.js with shared `_macros.html` (kpi_card, chart_container, data_table, metric_badge, empty_state).
  ...
  - **Tab list (in render order)**: ALCO Risk Summary (with embedded Decision Pack), Summary (with Locked-in NII KPI), CoC Decomposition, P&L Series, Shock Sensitivity (with sensitivity explain), EVE (IRRBB outlier test, tenor ladder, convexity/gamma, KRD), NII-at-Risk (parametric EaR), Repricing Gap, FX Mismatch, NMD Audit Trail (with replication portfolio), Deposits (beta validation, beta sensitivity ±0.1, concentration), Risk Cube, Regulatory Scorecard, Budget vs Actual, P&L Attribution (waterfall), Forecast Tracking, Strategy IAS, Counterparty, Hedge Effectiveness (scenario cross-ref), Hedge Strategy (DV01-based hedge recommendations), NIM (Jaws), Fixed/Float, Deal Explorer, Maturity Wall, FTP, Liquidity, Scenario Studio (NII+ΔEVE ranking, reverse stress, decision matrix), BOOK2 MTM, Rate Curves, Trends, Basis Risk (spread compression sensitivity), SNB Reserves (2.5% compliance), Peer Benchmark (FINMA aggregates), NMD Backtest (placeholder), Alerts, Data Quality. Note: `result["limits"]` and `result["alco_decision_pack"]` are computed but rendered inside other tabs (Limit Utilization → EVE/NII-at-Risk panels, Decision Pack → ALCO tab).
...
- **Refixing (floating deals)**: `build_rate_matrix` branches on `fixing_tenor_days` (resolved in `_resolve_rate_ref`). Tenor==0 → overnight RFR path: daily forward-curve interpolation + `LOOKBACK_DAYS` shift (SARON 2-BD, SONIA 5-BD). Tenor>0 (term floater, e.g. SARON3M/ESTR6M) → step-function path: for the segment containing today the rate is the contractual `current_fixing_rate` from MTD; future segments sample the forward curve at `last_fixing + k·tenor`. Tenor is inferred from `(next_fixing − last_fixing).days` first, else from a numeric suffix regex on the short name. The RFR-drop guard only removes deals with tenor==0 AND `ref_index ∈ CURRENCY_TO_OIS.values()`, so term floaters sharing an OIS base survive.
